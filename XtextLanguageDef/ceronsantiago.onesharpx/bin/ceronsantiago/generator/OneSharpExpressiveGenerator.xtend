...
class OneSharpExpressiveGenerator extends AbstractGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		var filename = resource.URI.lastSegment.replace(".osx","")
		projectedSize = 0
		//Calculate the size
		...
		//Print a pretty copy, a nonspace copy, and an intermediate language copy.
		var builder = new StringBuilder
		var uglyBuilder = new StringBuilder
		...
		for(i:0..<fin){
			...
			builder.append(SEP)
			//Ugly copy
			uglyBuilder.append(commandArray.get(i))
			//Intermediate language
			intBuilder.append(midLangArray.get(i)+SEP)
		}
		fsa.generateFile(filename+".os",uglyBuilder.toString)			
		fsa.generateFile(filename+"_pretty.os",builder.toString)
		fsa.generateFile(filename+".osi",intBuilder.toString)
	}
	...
	def dispatch resolve(Call cmd, int fpos){
		...
		for(var i = 0; i < pdecs.size; i ++){
			...
			if(decp instanceof IntParamDec){
				...
				if(calp.param != null){
					...
					//parameter context.
					pIval = Utils.isInt(parameterContextStack.peek().resolve(calp.param))
				}
				else{
				...
				if(calp.param != null){
					...
					pVal = parameterContextStack.peek().resolve(calp.param)
				}
				else{
					pVal = calp.stringVal
				}
		...
		println("Call: "+fdec.name)
		for(pair:pCtx.valMap.keySet){
			println(pair+" "+pCtx.valMap.get(pair))
		}
	...
	def dispatch resolve(NormalCase cmd, int fpos){
		...
		//Store the ends of each case to redirect in those to the end of the switch statement.
		var ends=newArrayList()
		//Resolve first case. Start at position pos+4 to leave room for other cases.
		pos = resolveCommandBlock(cmd.caseEmpty,fpos+4)
		//Save the end of the case to then tell it to go to the end of the switch. pos now has the end of case 1 implementation.
		ends.add(pos)
		pos++;
		//And in the cases declaration tell it where it will find the start of the case implementation.
		addMoveToArrays(fpos+1,fpos+4)
		//Do the same for the other cases.
			//Case #
		addMoveToArrays(fpos+2,pos)
		pos = resolveCommandBlock(cmd.caseOne,pos)
		ends.add(pos)
		pos++;
			//Case empty
		...
		ends.add(pos)
		pos++;
		//All the case ends should redirect to the end of the switch which is in pos.
		for(end:ends){
			addMoveToArrays(end,pos)
		}
		return pos
	}
	...
	def resolveCommandBlock(CommandBlock bloc, int fpos){
		...
		currentLabelContext = currentLabelContext.parent
		return pos
	}
