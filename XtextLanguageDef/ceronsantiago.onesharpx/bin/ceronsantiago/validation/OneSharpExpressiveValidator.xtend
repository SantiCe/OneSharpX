	  ...
	  */
	  ...
	  def checkCorrectTypeOfParams(CallParam param){
	  	...
	  	if(pos >= funparams.size) checkCorrectNumberOfParams(command)
	  		...
	  		if(pdec instanceof IntParamDec && !Utils.matchesType(param,pdec)){
	  			...
	  			if(!Utils.matchesType(param,pdec)){
	  ...
	  def private String paramVal(CallParam param){
	  	if(param.stringVal != null) return param.stringVal
	  	if(param.param != null) return param.param.name
	  	return param.intVal+""
	  }
	  /**
	   ...
	   */ 
	   	...
	   	def checkForNoCyclicCalls(Call command){
	   		...
	   		if(!dfs(command.function,gray,black)){
	   			error("Cyclic call detected. "+command.function.name,
	   				command,OneSharpExpressivePackage.Literals.CALL__FUNCTION,"Cycle detected"
	   			)
	   		}
	   	}
	   	
	   	def private boolean dfs(FunctionDeclaration fdec, HashSet<String> gray, HashSet<String> black){
	   		if(gray.contains(fdec.name)) return false
	   		gray.add(fdec.name)
	   		var list = new ArrayList<FunctionDeclaration> as List<FunctionDeclaration>
	   		list = functionsCalled(fdec.body,list)
	   	...
	   	def private List<FunctionDeclaration> functionsCalled(CommandBlock block, List<FunctionDeclaration> list){
	   		//Get direct calls.
	   		...
	   		{
	   			retList = cas.caseOne.functionsCalled(retList)
	   			retList = cas.caseEmpty.functionsCalled(retList)
	   			retList = cas.caseSharp.functionsCalled(retList)   				
	   		}
	   		return retList
	   	}		
}
