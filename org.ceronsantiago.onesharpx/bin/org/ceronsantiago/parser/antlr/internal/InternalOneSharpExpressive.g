...
entryRuleVAR_INT returns [String current=null] 
	:
	{ newCompositeNode(grammarAccess.getVAR_INTRule()); } 
	 iv_ruleVAR_INT=ruleVAR_INT 
	 { $current=$iv_ruleVAR_INT.current.getText(); }  
	 EOF 
;

// Rule VAR_INT
ruleVAR_INT returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
(    this_INT_0=RULE_INT    {
